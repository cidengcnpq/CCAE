...
OUTPUT_DIR = os.path.join(os.path.dirname(BASE_DIR), 'C:/Users/Alienware/Documents/Victor_Higino/optuna_optimization_output_Z24')

# --- Dataset settings ---
SENSORES = [f'Sensor{i}' for i in range(1, 8)]#7 sensores para Z24
...
class CWTPreloadedDataset(Dataset):
    def __init__(self, root_folder, sensor_names):
        ...
        # Defines the fixed transformations that will be applied only once
        # during preloading.
    ...
    def __getitem__(self, idx):
        # The image is already a tensor, so we don't need a transformation
